<template>
  <div>
    <t-layout class="_fc-designer" :style="'height:' + dragHeight">
      <t-aside style="padding: 0 10px; border-top: 1px solid #ececec">
        <div v-for="(item, index) in menuList" :key="index">
          <div class="_fc-l-group">
            <t-divider title-placement="left" class="_fc-l-title">{{
              item.title
            }}</t-divider>
            <draggable
      ...
      <t-layout class="_fc-m">
        <!-- 中间部分上方按钮组 -->
        <t-header class="_fc-m-tools" style="padding: 0 20px">
          ...
          <t-space>
            ...
            <t-popconfirm
              :cancel-btn="{
                content: t('designer.clearCancel'),
                size: 'small'
              }"
              :confirm-btn="{
                content: t('designer.clearConfirm'),
                size: 'small'
              }"
            ...
            >
              ...
              <t-button theme="warning" shape="round" ghost size="small">
        ...
        <!-- 中间拖拽组件实现部分 -->
        <t-content style="background: #f5f5f5; padding: 20px">
      ...
      >
        <div style="height: 100%; position: relative">
          <t-layout style="width: 100%" class="_fc-r-tabs-wrap" position="absolute">
            <!-- 右侧边栏标题 -->
            <t-header
              style="height: 40px"
              class="_fc-r-tabs"
    ...
    const methods = {
      ...
      toolActive(rule) {
        ...
        if (data.showBaseRule) {
          ...
          data.validateForm.options.formData = {
...
._fc-r ._fc-r-tabs ._fc-r-tab.active {
  color: var(--td-success-color);
  font-weight: var(--n-tab-font-weight-active);
  border-bottom: 2px solid var(--td-success-color);
}
